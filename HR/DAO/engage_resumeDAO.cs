...
namespace DAO
{
    public class engage_resumeDAO:DaoBase<engage_resume>,Iengage_resumeDAO
    {
        ...
        public List<engage_resumeModel> SeBy(String Qid, String Pid, String Guan, String Start, String End)
        {
            ...
            if (Start == "" && End == "")
            {
                list = SelectBy(e => e.human_major_kind_id.Contains(Qid) && e.human_major_id.Contains(Pid) && (e.human_name.Contains(Guan) || e.human_telephone.Contains(Guan) || e.human_idcard.Contains(Guan) || e.human_history_records.Contains(Guan)));
            }
            else
            {
                DateTime start = Convert.ToDateTime(Start);
                DateTime end = Convert.ToDateTime(End);
                list = SelectBy(e => e.human_major_kind_id.Contains(Qid) && e.human_major_id.Contains(Pid) && (e.human_name.Contains(Guan) || e.human_telephone.Contains(Guan) || e.human_idcard.Contains(Guan) || e.human_history_records.Contains(Guan)) && e.regist_time >= start && e.regist_time <= end);
            }
                List<engage_resumeModel> list2 = new List<engage_resumeModel>();
            foreach (var item in list)
            {
