...
namespace DAO
{
    public class engage_resumeDAO:DaoBase<engage_resume>,Iengage_resumeDAO
    {
        ...
        public List<engage_resumeModel> SeBy(int dqy ,String Qid, String Pid, String Guan, String Start, String End)
        {
            ...
            if (Start == "" && End == "")
            {
                list = FenYe(e => e.Id,e => e.human_major_kind_id.Contains(Qid) && e.human_major_id.Contains(Pid) && (e.human_name.Contains(Guan) || e.human_telephone.Contains(Guan) || e.human_idcard.Contains(Guan) || e.human_history_records.Contains(Guan)), ref rows, dqy, 3);
            }
            else
            {
